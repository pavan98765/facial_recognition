...
parser.add_argument(
    "--tolerance",
    type=float,
    default=0.5,  # Default tolerance value
...
def recognize_faces(image, loaded_encodings):
    image = cv2.resize(
        image,
        None,
        fx=args.size_ratio,
    ...
    for bounding_box, unknown_encoding in zip(
        ...
        if not name:
            name = "unknown"
        _display_face(image, bounding_box, name)
...
def validate():
    ...
    for filepath in Path("validation").rglob("*"):
        if filepath.is_file():
            ...
            image_with_faces = recognize_faces(image, loaded_encodings)
...
if __name__ == "__main__":
    ...
    if args.live:
        ...
        while True:
            ...
            frame_with_faces = recognize_faces(frame, loaded_encodings)
